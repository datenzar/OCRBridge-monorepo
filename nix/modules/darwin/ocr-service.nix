  ...
  programArguments =
    if cfg.apiKeysFile == null then
      ...
      [
        ...
        ''
          set -a
          . ${lib.escapeShellArg (pathToString cfg.apiKeysFile)}
          set +a
...
in
{
  options.ocrbridge.ocr-service = {
    ...
    flavor = mkOption {
      type = types.enum [ "lite" "full" "macos" ];
      default = "lite";
  ...
  config = mkIf cfg.enable {
    ...
    launchd.daemons.ocr-service.serviceConfig = {
      ...
      StandardOutPath = "/var/log/ocr-service.log";
      StandardErrorPath = "/var/log/ocr-service.err.log";
    };
  };
}
